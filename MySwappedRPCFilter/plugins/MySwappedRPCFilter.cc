...
MySwappedRPCFilter::MySwappedRPCFilter(const edm::ParameterSet& iConfig)
{
  ...
  selectRPChit         = iConfig.getUntrackedParameter<bool>("SelectRPChit", false);
  selectMuon           = iConfig.getUntrackedParameter<bool>("SelectMuon", false);
  selectSegment        = iConfig.getUntrackedParameter<bool>("SelectSegment", false);
  selectOR             = iConfig.getUntrackedParameter<bool>("SelectOR", false);
  selectAND            = iConfig.getUntrackedParameter<bool>("SelectAND", false);
...
MySwappedRPCFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(selectSegment) {
    // ==================================================================================
    // = Check whether there is a CSC or DT Segment pointing to any of the RPC Rolls    =
    // ==================================================================================
    if(invest_region == 0) {
	...
	DTChamberId chamberId = segment->chamberId();
	if(invest_ring == chamberId.wheel()) {
	  if(invest_station == chamberId.station()) {
	    if(invest_sector == chamberId.sector()) {
	      keepSegment = true;
	      if(debug) {
		std::cout<<" DT DetId: "<<std::setw(12)<<chamberId.rawId()<<" details: "<<chamberId;
		std::cout<<" DT Segment at position "<<segment->localPosition()<<" and with direction "<<segment->localDirection();
		std::cout<<std::endl;
	      }
	    } 
	  }
      ...
      // loop over CSC segments ... to be implemented
    }
  }
  ...
  // ==================================================== 
  if(selectAND) {
    if((selectMuon    && keepStaMuon) && (selectRPChit && keepRPCRecHit)) { keepEvent=true; ++eventsFiltered; }
    if((selectSegment && keepSegment) && (selectRPChit && keepRPCRecHit)) { keepEvent=true; ++eventsFiltered; }
  }
  if(selectOR) {
    if((selectMuon    && keepStaMuon) || (selectRPChit && keepRPCRecHit)) { keepEvent=true; ++eventsFiltered; }
    if((selectSegment && keepSegment) || (selectRPChit && keepRPCRecHit)) { keepEvent=true; ++eventsFiltered; }
